...
export const DocumentRegistryABI = [
  ...
  "function createDocument(bytes32 _docId, string _ipfsCid, bytes32 _encryptedKeyHash, bytes32 _contentHash)",
  "function createVersion(bytes32 _docId, string _ipfsCid, bytes32 _encryptedKeyHash, bytes32 _contentHash)",
  ...
  "function getVersion(bytes32 _docId, uint256 _versionNumber) view returns (tuple(uint256 versionNumber, string ipfsCid, bytes32 encryptedKeyHash, bytes32 contentHash, address createdBy, uint256 createdAt, bool isOperational, uint256 restoredFrom))",
...
export const CONTRACTS: Record<string, ContractConfig> = {
  DocumentRegistry: {
    address: getPersistedDocumentRegistryAddress() || import.meta.env.VITE_CONTRACT_REGISTRY || '0x86A2EE8FAf9A840F7a2c64CA3d51209F9A02081D',
...
export function setContractAddress(name: string, address: string): void {
  ...
  if (!contract) {
    throw new Error(`Contrato ${name} no encontrado en configuración`);
  }

  if (!isEthereumAddress(address)) {
    throw new Error(`Dirección Ethereum inválida para ${name}`);
  }
