...
export type WalletType = 'metamask' | 'coinbase' | 'brave' | 'trust' | 'other';
...
export class BlockchainProvider {
  ...
  static async addHardhatNetwork(): Promise<void> {
    ...
    if (!ethereum) {
      throw new Error('No se encontró proveedor Ethereum');
    }
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") { console.error('Error adding Hardhat network:', error); }
      throw error;
    }
  }
  ...
  async connectWallet(type: WalletType = 'metamask', provider?: unknown): Promise<WalletConnection> {
    switch (type) {
      ...
      default:
        throw new Error(`Tipo de wallet no soportado: ${type}`);
    }
  }
  ...
  private async connectBrowserWallet(type: WalletType, specificProvider?: unknown): Promise<WalletConnection> {
    ...
    if (!ethereum) {
      throw new Error('No se encontró una wallet Ethereum. Instala MetaMask o similar.');
    }
    ...
    try {
      ...
      if (!accounts || accounts.length === 0) {
        throw new Error('No se encontraron cuentas. Desbloquea tu wallet.');
      }
      ...
      if (process.env.NODE_ENV === "development") { console.error(`${type} connection error:`, error); }
      throw error;
    }
  }
  ...
  private handleChainChanged = (chainId: unknown) => {
      ...
      this.provider?.getSigner().then(s => {
        this.signer = s;
      }).catch(() => { /* best effort — signer refresh is not critical */ });
    };
  ...
  private async addNetwork(chainId: number): Promise<void> {
    ...
    if (!networkConfig) {
      throw new Error(`Configuración de red no encontrada para chain ID ${chainId}`);
    }
  ...
  async getGasPrice(): Promise<bigint> {
    if (!this.provider) {
      throw new Error('Proveedor no conectado');
    }
  ...
  async signMessage(message: string): Promise<string> {
    if (!this.signer) {
      throw new Error('Firmante no disponible');
    }
