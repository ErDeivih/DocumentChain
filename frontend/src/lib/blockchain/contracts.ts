...
import { ethers, Contract, TransactionResponse, BytesLike } from 'ethers';
...
export class DocumentRegistryContract extends BaseContract {
  ...
  // ── escritura: ciclo de vida de documentos ────────────────────────────────
  ...
  async createDocument(
    ...
    contentHash: string = ethers.ZeroHash,
  ): Promise<TransactionResponse> {
    return this.contract.createDocument(this.toBytes32(docId), ipfsCid, encryptedKeyHash ?? ethers.ZeroHash, contentHash ?? ethers.ZeroHash);
  }
  ...
  async createVersion(
    ...
    return this.contract.createVersion(this.toBytes32(docId), ipfsCid, encryptedKeyHash ?? ethers.ZeroHash, contentHash ?? ethers.ZeroHash);
  }
  ...
  // ── escritura: permisos y compartición ────────────────────────────────────
  ...
  async shareDocument(
    ...
    if (!ethers.isAddress(userAddress)) throw new Error('Dirección Ethereum inválida');
  ...
  async transferOwnership(docId: string, newOwner: string): Promise<TransactionResponse> {
    if (!ethers.isAddress(newOwner)) throw new Error('Dirección Ethereum inválida');
