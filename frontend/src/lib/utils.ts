...
export async function copyToClipboard(text: string): Promise<void> {
  ...
  throw new Error('No se pudo copiar al portapapeles — navigator.clipboard no disponible en este entorno');
}

/**
 * Ejecuta una promesa con un timeout máximo.
 * Si la promesa no se resuelve en `ms` milisegundos, rechaza con el mensaje indicado.
 *
 * @param promise - Promesa a ejecutar.
 * @param ms - Tiempo máximo en milisegundos.
 * @param message - Mensaje de error si se supera el timeout.
 * @returns Resultado de la promesa original.
 */
export function withTimeout<T>(promise: Promise<T>, ms: number, message = 'La operación excedió el tiempo máximo'): Promise<T> {
  return Promise.race([
    promise,
