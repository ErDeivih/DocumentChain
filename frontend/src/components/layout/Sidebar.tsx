          ...
          <div className="max-h-64 overflow-y-auto">
            <FolderTree
              ...
              onDeleteFolder={async (folder) => {
                if (confirm(`¿Eliminar la carpeta "${folder.name}"? Los documentos se moverán a la raíz.`)) {
                  try { await deleteFolder(folder.id, true); window.dispatchEvent(new Event('foldersChanged')); }
                  catch { /* silently handled by UI refresh */ }
                }
              }}
            />
          </div>
