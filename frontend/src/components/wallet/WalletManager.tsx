...
/**
 * Panel de gestion de wallets. Permite conectar, renombrar, establecer como
 * principal y eliminar wallets blockchain mediante el navegador (MetaMask).
 */
export const WalletManager: React.FC<WalletManagerProps> = ({
  ...
  const handleConnect = async (_method: 'browser') => {
    setShowConnectionMethod(false);
    try {
      const type = 'metamask' as const;
