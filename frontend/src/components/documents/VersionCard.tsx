...
import { CheckCircle, FileSignature, Download, ArrowRight, RotateCcw, Loader2 } from 'lucide-react';
...
/**
 * Tarjeta que muestra una versión de documento con sus acciones disponibles.
 *
 * Permite ver firmantes, descargar y, si el usuario es propietario, activar
 * una versión como operacional. Muestra distintivos visuales para la versión activa.
 */
  ...
  >
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            {isOperational && <Badge variant="success" className="text-xs">Activa</Badge>}
      ...
      <div className="flex items-center gap-2">
        ...
        {isOwner && !isOperational && onRestore && (
          <Button variant="outline" size="sm" onClick={() => onRestore(version.versionNumber)} disabled={changing !== null || isArchived} title={isArchived ? 'No se puede restaurar una versión de un documento archivado' : `Restaurar como nueva versión operativa`}>
            {changing === version.versionNumber ? <Loader2 className="h-4 w-4 animate-spin" /> : <><RotateCcw className="h-4 w-4 mr-1" />Restaurar</>}
          </Button>
        )}
        {isOwner && !isOperational && !onRestore && (
