...
export const RecoveryKeyDisplay: React.FC<RecoveryKeyDisplayProps> = ({
  ...
  const handleCopy = async () => {
    try {
      ...
      if (process.env.NODE_ENV === "development") { console.error('Error al copiar:', error); }
    }
  };
  ...
  return (
    ...
    <Dialog open={showCloseWarning} onOpenChange={setShowCloseWarning}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>¿Cerrar sin guardar la clave?</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <AlertMessage
            type="error"
            message="Si cierra esta ventana sin guardar su clave de recuperación, NO podrá recuperar su cuenta si olvida su contraseña."
          />
          <p className="text-sm text-muted-foreground">
            Sus documentos cifrados serán <strong>permanentemente inaccesibles</strong> si pierde el acceso a su cuenta.
          </p>
