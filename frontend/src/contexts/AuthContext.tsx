...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const logout = useCallback(async () => {
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") { console.error('Error al cerrar sesión:', error); }
    } finally {
      SecureStorage.clearAll();
  ...
  const refreshUser = useCallback(async () => {
    try {
      ...
      if (process.env.NODE_ENV === "development") { console.error('Error al refrescar usuario:', error); }
      await logout();
      window.location.href = '/login';
    }
  }, [logout]);
   ...
   */
  const patchUserSession = useCallback((patch: Partial<User> | ((currentUser: User) => User)) => {
    setUser((currentUser) => {
      if (!currentUser) {
        return currentUser;
      }
