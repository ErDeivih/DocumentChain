...
export function WalletManagerProvider({ children }: { children: ReactNode }) {
          ...
          try {
            ...
            if (accounts && accounts.length > 0) {
              try {
                ...
                setConnectedWallet({ address: accounts[0].toLowerCase(), chainId, type: 'metamask' });
              } catch {
                setConnectedWallet({ address: accounts[0].toLowerCase(), chainId: 1, type: 'metamask' });
              }
            }
          } catch { /* silencioso - el usuario aun no ha autorizado */ }
        })();
    ...
    const handleAccountsChanged = async (accounts: string[]) => {
      if (accounts.length === 0) {
        ...
        setConnectedWallet(prev => {
          if (prev) return { ...prev, address: accounts[0].toLowerCase() };
          return { address: accounts[0].toLowerCase(), chainId: 1, type: 'metamask' };
        });
      }
    };
  ...
  const loadSavedWallets = useCallback(async () => {
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") { console.error('Error cargando wallets:', err); }
  ...
  /**
   * Conecta una wallet externa (MetaMask) para firmar transacciones.
   *
   * @param type - Tipo de proveedor de wallet.
  ...
  const connectWallet = useCallback(async (type: WalletType, provider?: unknown): Promise<ConnectedWallet> => {
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") { console.error('Error conectando wallet:', err); }
      try { await blockchainProvider.disconnect(); } catch { /* best effort */ }
      setConnectedWallet(null);
  ...
  const addWallet = useCallback(async (label?: string): Promise<SavedWallet> => {
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") { console.error('Error añadiendo wallet:', err); }
  ...
  const removeWallet = useCallback(async (walletId: string) => {
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") { console.error('Error eliminando wallet:', err); }
  ...
  const setPrimaryWalletFn = useCallback(async (walletId: string) => {
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") { console.error('Error estableciendo wallet principal:', err); }
