...
import { type Folder, moveDocumentsToFolder } from '../api/folders';
...
import { Upload, FileText, FolderPlus, Search, Filter, Loader2, FolderInput, X } from 'lucide-react';
...
export const Documents: React.FC = () => {
  ...
  const handleBatchMove = async () => {
    ...
    setIsMoving(true);
    try {
      await moveDocumentsToFolder(targetFolderId, [...selectedIds]);
  ...
  return (
    <div className="space-y-6">
          ...
          <>
            {selectedIds.size > 0 && (
              <div className="flex items-center gap-3 rounded-lg border border-primary/30 bg-primary/5 px-4 py-2">
                <button onClick={handleClearSelection} className="text-muted-foreground hover:text-foreground" title="Cancelar selección">
                  <X className="w-4 h-4" />
                </button>
      ...
      <Dialog open={showMoveModal} onOpenChange={setShowMoveModal}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Mover {selectedIds.size} documento{selectedIds.size !== 1 ? 's' : ''}</DialogTitle>
          </DialogHeader>
          <FolderSelector
          ...
          />
          <DialogFooter>
            <Button variant="ghost" onClick={() => setShowMoveModal(false)}>Cancelar</Button>
            <Button variant="primary" onClick={handleBatchMove} disabled={!targetFolderId || isMoving} isLoading={isMoving}>
              Mover
            </Button>
          </DialogFooter>
        </DialogContent>
