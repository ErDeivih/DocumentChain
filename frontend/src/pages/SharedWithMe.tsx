            ...
            <div className="mt-2 flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <span>
                ...
                <span className="font-medium text-foreground">
                  {activeWallet.label || 'Cartera sin nombre'}
                </span>{' '}
                      ...
                      {doc.tags && doc.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1" onClick={(e) => e.stopPropagation()}>
                          {doc.tags.slice(0, 3).map((tag: string, idx: number) => (
                            <button key={idx} onClick={(e) => { e.preventDefault(); setSearchInput(tag); }} className="focus:outline-none">
                              <Badge variant="secondary" className="cursor-pointer hover:bg-primary/20 transition-colors text-xs">{tag}</Badge>
                            </button>
                          ))}
                          {doc.tags.length > 3 && (
