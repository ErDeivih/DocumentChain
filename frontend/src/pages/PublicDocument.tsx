...
export const PublicDocument: React.FC = () => {
  ...
  const { data: auditTrail } = useQuery({
    ...
    queryFn: () => {
      if (!document?.blockchainId) throw new Error('Documento no disponible');
      return auditApi.getAuditTrail(document.blockchainId);
    },
  ...
  const downloadUrl = selectedVersion
    ? publicDocumentsApi.getDownloadUrl(document.publicId, selectedVersion.versionNumber)
    : null;
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 mb-4 text-muted-foreground">
            Documento público: el contenido se almacena sin cifrar y cualquier persona con el enlace puede verlo o descargarlo.
          </p>
        </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-3 text-sm">
              <div>
                ...
                <p className="font-medium">{document.owner?.fullName || document.owner?.username || 'Desconocido'}</p>
              </div>
              <div>
            ...
            <CardContent>
              <div className="space-y-3">
                {document.versions.length === 0 ? (
                  <p className="text-sm text-muted-foreground">No hay versiones disponibles para este documento.</p>
                ) : (
                  ...
                  <button
                    ...
                    className={`w-full rounded-lg border p-3 text-left transition-colors ${
                      version.versionNumber === selectedVersion?.versionNumber
                        ...
                        : 'border-gray-200 hover:border-primary/40 hover:bg-blue-50'
                    }`}
                    onClick={() => navigate(`/public/d/${document.publicId}/v/${version.versionNumber}`)}
                  >
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <p className="font-medium" style={{ color: '#111827' }}>Versión {version.versionNumber}</p>
                        <p className="text-xs" style={{ color: '#6b7280' }}>{formatRelativeTime(version.createdAt)}</p>
                      </div>
                      {version.versionNumber === document.operationalVersionNumber ? <Badge variant="success">Activa</Badge> : null}
                    </div>
                    {version.comment ? <p className="mt-2 text-sm" style={{ color: '#6b7280' }}>{version.comment}</p> : null}
                  </button>
                ))
                )}
              </div>
