...
export const Login: React.FC = () => {
  ...
  const from = (location.state as { from?: { pathname: string } })?.from?.pathname || '/app/documents';
  const attemptedIdentifier = username.trim();
  const loginNeedsEmailVerification = !!error && /verificar/i.test(error);
        ...
        <CardContent>
          ...
          <div className="mt-4 text-center">
            <Link to="/forgot-password" className="text-sm text-primary hover:underline font-medium">
              ¿Olvidó su contraseña o accede desde otro dispositivo?
            </Link>
          </div>
