...
/**
 * Página de configuración de la cuenta de usuario.
 *
 * Permite gestionar el perfil (avatar, nombre, email), cambiar la contraseña,
 * configurar preferencias de notificación, administrar wallets y eliminar la cuenta.
 * Organizada en pestañas: Perfil, Seguridad y Cuenta, Wallets.
 */
export const Settings: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadNotificationPrefs = async () => {
      try {
        ...
        if (import.meta.env.DEV) { console.warn('Error al cargar preferencias de notificación:', err); }
      }
    };
  ...
  const updateProfileMutation = useMutation({
    ...
    onError: (error: unknown) => {
      ...
      timerRef.current = setTimeout(() => setShowError(null), 5000);
    },
  });

  const changePasswordMutation = useMutation({
    mutationFn: async (data: { currentPassword: string; newEncryptedPrivateKey: string; newSalt: string; newPassword: string }) => {
      await authApi.changePassword(data.currentPassword, data.newEncryptedPrivateKey, data.newSalt, data.newPassword);
    },
    onSuccess: async () => {
  ...
  const handleSaveProfile = () => {
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim())) {
      setShowError('Por favor, introduce un email válido.');
      return;
    }
  ...
  /**
   * Cambia la contraseña del usuario. Descifra la clave privada con la contraseña actual
   * y la re-cifra con la nueva contraseña, todo en el navegador.
   * La clave privada nunca sale del navegador ni se expone al servidor.
   */
  const handleChangePassword = async () => {
    ...
    if (!/[A-Z]/.test(passwordForm.newPassword) || !/[a-z]/.test(passwordForm.newPassword) || !/\d/.test(passwordForm.newPassword)) {
      setShowError('La contraseña debe incluir al menos una mayúscula, una minúscula y un número.');
      return;
    }

    if (!user?.encryptedPrivateKey) {
      setShowError('No se encontró la clave privada cifrada. Inicie sesión de nuevo.');
      return;
    }
    ...
    try {
      ...
      setShowError('Error al descifrar la clave privada. Verifique su contraseña actual.');
    }
  };
