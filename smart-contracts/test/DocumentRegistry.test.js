...
describe("DocumentRegistry - Complete Test Suite", function () {
  ...
  async function deployWithDocumentFixture() {
    ...
    // Create a document
    await registry.connect(owner).createDocument(docId, ipfsCid, encryptedKeyHash, ethers.ZeroHash);
  ...
  async function deployWithVersionsFixture() {
    ...
    await registry.connect(owner).createVersion(docId, "QmVersion2", ethers.id("key2"), ethers.ZeroHash);
    await registry.connect(owner).createVersion(docId, "QmVersion3", ethers.id("key3"), ethers.ZeroHash);
  ...
  describe("2. Document Creation", function () {
    it("2.1 Should create document with version 1", async function () {
      ...
      await expect(registry.connect(owner).createDocument(docId, ipfsCid, encryptedKeyHash, ethers.ZeroHash))
    ...
    it("2.7 Should revert if document already exists", async function () {
      ...
      await expect(
        registry.connect(owner).createDocument(docId, ipfsCid, encryptedKeyHash, ethers.ZeroHash)
    ...
    it("2.8 Should revert with empty IPFS CID", async function () {
      ...
      await expect(
        registry.connect(owner).createDocument(docId, "", encryptedKeyHash, ethers.ZeroHash)
  ...
  describe("3. Version Management", function () {
    describe("3.1 Creating New Versions", function () {
      it("3.1.1 Should create new version with correct number", async function () {
        ...
        await expect(registry.connect(owner).createVersion(docId, "QmVersion2", ethers.id("key2"), ethers.ZeroHash))
          .to.emit(registry, "VersionCreated")
          .withArgs(docId, 2, "QmVersion2", ethers.ZeroHash, owner.address, anyValue);
      });
      ...
      it("3.1.6 Should revert if not OWNER or EDITOR", async function () {
        ...
        await expect(
          registry.connect(user1).createVersion(docId, "QmVersion2", ethers.id("key2"), ethers.ZeroHash)
      ...
      it("3.1.7 Should allow EDITOR to create version", async function () {
        ...
        await expect(registry.connect(user1).createVersion(docId, "QmVersion2", ethers.id("key2"), ethers.ZeroHash))
          .to.emit(registry, "VersionCreated");
      });
    });
  ...
  describe("4. Document Signing", function () {
    ...
    it("4.4 Should allow same user to sign different versions", async function () {
      ...
      await registry.connect(owner).shareDocument(docId, user1.address, 1);
      await registry.connect(owner).createVersion(docId, "QmV2", ethers.id("key2"), ethers.ZeroHash);
    ...
    it("4.11 Should reject replay on another document", async function () {
      ...
      await registry.connect(owner).shareDocument(docId, user1.address, 1);
      await registry.connect(owner).createDocument(otherDocId, "QmOther", encryptedKeyHash, ethers.ZeroHash);
  ...
  describe("5. Permissions & Sharing", function () {
    ...
    describe("5.3 View Functions", function () {
      ...
      it("5.3.9 Should get all user documents", async function () {
        ...
        await registry.connect(owner).createDocument(docId1, "QmCid1", ethers.id("key1"), ethers.ZeroHash);
        await registry.connect(owner).createDocument(docId2, "QmCid2", ethers.id("key2"), ethers.ZeroHash);
  ...
  describe("10. View Functions", function () {
    ...
    it("10.6 Should get user document count", async function () {
      ...
      await registry.connect(owner).createDocument(ethers.id("doc1"), "QmCid1", ethers.id("key1"), ethers.ZeroHash);
      await registry.connect(owner).createDocument(ethers.id("doc2"), "QmCid2", ethers.id("key2"), ethers.ZeroHash);
  ...
  describe("11. Security & Edge Cases", function () {
    ...
    it("11.4 Should handle document with many versions", async function () {
      ...
      for (let i = 2; i <= 10; i++) {
        await registry.connect(owner).createVersion(docId, `QmVersion${i}`, ethers.id(`key${i}`), ethers.ZeroHash);
      }
  ...
  // ============================================
  // 12. EDGE CASES & LIFE CYCLE (4 tests)
  // ============================================

  describe("12. Edge Cases & Lifecycle", function () {
    it("12.1 Should accept valid CID on createVersion", async function () {
      const { registry, owner, docId, ipfsCid } = await loadFixture(deployWithDocumentFixture);

      await registry.connect(owner).createVersion(docId, ipfsCid, ethers.id("empty-key"), ethers.ZeroHash);
    ...
    it("12.2 Should preserve permissions after ownership transfer", async function () {
      const { registry, owner, user1, user2, docId } = await loadFixture(deployWithDocumentFixture);

      await registry.connect(owner).shareDocument(docId, user2.address, 2);
      await registry.connect(owner)['transferOwnership(bytes32,address)'](docId, user1.address);

      expect(await registry.canEdit(docId, user2.address)).to.be.true;
      expect(await registry.isOwner(docId, user1.address)).to.be.true;
      expect(await registry.getUserPermission(docId, user1.address)).to.equal(3);
    });

    it("12.3 Should return correct state after soft delete", async function () {
      const { registry, owner, docId, ipfsCid } = await loadFixture(deployWithDocumentFixture);
      ...
      const v1 = await registry.getVersion(docId, 1);
      expect(v1.ipfsCid).to.equal(ipfsCid);
    ...
    it("12.4 Should handle archive → unarchive → delete lifecycle sequence", async function () {
      const { registry, owner, docId } = await loadFixture(deployWithDocumentFixture);

      await registry.connect(owner).setArchiveStatus(docId, true);
      let doc = await registry.getDocument(docId);
      expect(doc.isArchived).to.be.true;

      await registry.connect(owner).setArchiveStatus(docId, false);
      doc = await registry.getDocument(docId);
      expect(doc.isArchived).to.be.false;
  ...
  // ============================================
  // 13. LEGACY — features removed from contract
  ...
  describe("13: Additional Revert Tests", function () {
    it("should not revert when archiving an already archived document (no-op)", async function () {
      const { registry, owner } = await loadFixture(deployDocumentRegistryFixture);
      const docId = ethers.encodeBytes32String("doc-archive-twice");
      await registry.createDocument(docId, "QmTest1", ethers.ZeroHash, ethers.ZeroHash);
      await registry.setArchiveStatus(docId, true);
      await registry.setArchiveStatus(docId, true); // No-op
      const doc = await registry.getDocument(docId);
      expect(doc.isArchived).to.be.true;
    });

    it("should not revert when unarchiving a non-archived document (no-op)", async function () {
      const { registry, owner } = await loadFixture(deployDocumentRegistryFixture);
      const docId = ethers.encodeBytes32String("doc-unarchive-fresh");
      await registry.createDocument(docId, "QmTest1", ethers.ZeroHash, ethers.ZeroHash);
      await registry.setArchiveStatus(docId, false); // No-op
      const doc = await registry.getDocument(docId);
      expect(doc.isArchived).to.be.false;
    });
