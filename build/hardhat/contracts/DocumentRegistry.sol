...
contract DocumentRegistry is AccessControl, ReentrancyGuard {
    ...
    /// @dev Separador de dominio EIP-191 para prevenir reúso de firmas entre contratos/redes.
     ...
     * @param createdAt Marca temporal de creación (marca temporal Unix).
     ...
     * @param ipfsCid Identificador de contenido (CID) de IPFS donde reside el contenido cifrado.
     * @param encryptedKeyHash Hash de la clave simétrica cifrada (para verificación de integridad).
     * @param contentHash Hash SHA-256 del contenido del archivo original, almacenado on-chain para verificación de integridad.
    ...
    /// @param docId Identificador único del documento (bytes32).
    /// @param owner Dirección del propietario.
    /// @param ipfsCid Identificador de contenido en IPFS.
    ...
    /// @param versionNumber Número de versión creada.
    /// @param ipfsCid Identificador de contenido en IPFS de la versión.
    /// @param contentHash Hash SHA-256 del contenido del archivo original, almacenado on-chain para verificación.
    /// @param createdBy Dirección del creador de la versión.
    /// @param timestamp Marca temporal de creación.
    event VersionCreated(bytes32 indexed docId, uint256 indexed versionNumber, string ipfsCid, bytes32 contentHash, address indexed createdBy, uint256 timestamp);

    /// @notice Emite cuando se restaura una versión anterior como nueva versión operativa.
    /// @param docId Identificador único del documento.
    /// @param newVersionNumber Nuevo número de versión generado por la restauración.
    /// @param restoredFromVersion Número de versión de origen restaurada.
    /// @param by Dirección que ejecutó la restauración.
    /// @param timestamp Marca temporal de la restauración.
    ...
    /// @param docId Identificador único del documento.
    /// @param versionNumber Número de versión firmada.
    /// @param signer Dirección del firmante.
    /// @param message Mensaje firmado (incluye hash, nonce y dirección del contrato).
    /// @param timestamp Marca temporal de la firma.
    ...
    /// @param docId Identificador único del documento.
    /// @param from Dirección del propietario que comparte.
    /// @param to Dirección del destinatario.
    /// @param role Rol asignado (VIEWER o EDITOR).
    /// @param timestamp Marca temporal de la compartición.
    ...
    /// @param docId Identificador único del documento.
    /// @param user Dirección del usuario cuyo permiso se revoca.
    /// @param by Dirección que ejecutó la revocación.
    /// @param timestamp Marca temporal de la revocación.
    ...
    /// @param docId Identificador único del documento.
    /// @param from Dirección del propietario anterior.
    /// @param to Dirección del nuevo propietario.
    /// @param timestamp Marca temporal de la transferencia.
    ...
    /// @param docId Identificador único del documento.
    /// @param by Dirección que ejecutó la acción.
    /// @param archived true si se archiva, false si se desarchiva.
    /// @param timestamp Marca temporal del cambio.
    ...
    /// @param docId Identificador único del documento.
    /// @param by Dirección que ejecutó la eliminación.
    /// @param timestamp Marca temporal de la eliminación.
    ...
    /// @param docId Identificador único del documento.
    /// @param oldVersion Número de la versión operativa anterior.
    /// @param newVersion Número de la nueva versión operativa.
    /// @param by Dirección que ejecutó el cambio.
    /// @param timestamp Marca temporal del cambio.
    ...
    /// @param admin Dirección que recibe el rol de administrador.
    /// @param by Dirección que concedió el rol.
    /// @param timestamp Marca temporal de la concesión.
    ...
    /// @param admin Dirección a la que se le revoca el rol.
    /// @param by Dirección que ejecutó la revocación.
    /// @param timestamp Marca temporal de la revocación.
     ...
     * @param _contentHash Hash SHA-256 del contenido del archivo. Se almacena on-chain en la estructura Version y se emite en el evento.
     * @dev Requiere: ID válido, CID no vacío y documento no existente previamente.
     */
    function createDocument(bytes32 _docId, string calldata _ipfsCid, bytes32 _encryptedKeyHash, bytes32 _contentHash)
        external
        nonReentrant
    {
        ...
        emit VersionCreated(_docId, 1, _ipfsCid, _contentHash, sender, block.timestamp);
    }
     ...
     */
    function createVersion(bytes32 _docId, string calldata _ipfsCid, bytes32 _encryptedKeyHash, bytes32 _contentHash)
        external
        nonReentrant
        notDeleted(_docId)
    {
        ...
        emit VersionCreated(_docId, newVersionNum, _ipfsCid, _contentHash, _msgSender(), block.timestamp);
    }
     ...
     *      La firma debe tener exactamente 65 bytes y se valida criptográficamente mediante ECDSA recovery.
     */
    ...
    /**
     * @notice Valida que una firma fue producida por el firmante esperado.
     * @param _docId Identificador del documento firmado.
     * @param _versionNumber Número de versión del documento.
     * @param _message Mensaje adicional incluido en la firma.
     * @param _signature Firma ECDSA a validar.
     * @param _expectedSigner Dirección que se espera haya firmado.
     */
     ...
     * @dev Solo el propietario puede modificar este estado. Un documento archivado no admite
     *      modificaciones de contenido, pero puede ser desarchivado por el propietario.
     */
     ...
     *      No se puede eliminar un documento archivado (debe desarchivarse primero).
     */
    ...
    /**
     * @notice Verifica si un usuario tiene al menos permiso de lectura.
     * @dev Comprueba el mapping de permisos para el documento y usuario dados.
     * @param _docId Identificador del documento en blockchain.
     * @param _user Dirección del usuario a verificar.
     * @return true si el usuario tiene VIEWER, EDITOR o OWNER.
     */
    ...
    /**
     * @notice Verifica si un usuario tiene permiso de escritura.
     * @dev Comprueba el mapping de permisos para el documento y usuario dados.
     * @param _docId Identificador del documento en blockchain.
     * @param _user Dirección del usuario a verificar.
     * @return true si el usuario tiene EDITOR o OWNER.
     */
    ...
    /**
     * @notice Verifica si una dirección es la propietaria del documento.
     * @dev Compara la dirección con el owner almacenado en la estructura Document.
     * @param _docId Identificador del documento en blockchain.
     * @param _user Dirección del usuario a verificar.
     * @return true si el usuario es el propietario.
     */
