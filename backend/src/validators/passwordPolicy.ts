...
export function generateSecurePassword(length: number = 16): string {
  ...
  const cryptoRandomIndex = (max: number): number => {
    const randomBytes = cryptoLib.randomBytes(4);
  ...
  password += uppercase[cryptoRandomIndex(uppercase.length)];
  password += lowercase[cryptoRandomIndex(lowercase.length)];
  password += numbers[cryptoRandomIndex(numbers.length)];
  password += special[cryptoRandomIndex(special.length)];

  // Rellenar el resto con bytes criptográficos
  for (let i = password.length; i < length; i++) {
    password += allChars[cryptoRandomIndex(allChars.length)];
  }

  // Mezclar caracteres (Fisher-Yates shuffle con crypto)
