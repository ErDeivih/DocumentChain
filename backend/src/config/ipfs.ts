...
const IPFS_PROVIDER = env.IPFS_PROVIDER;
const IPFS_API_URL = env.IPFS_API_URL;
const IPFS_GATEWAY_URL = env.IPFS_GATEWAY_URL;
const PINATA_JWT = normalizeEnvValue(env.PINATA_JWT, '');
const PINATA_API_KEY = normalizeEnvValue(env.PINATA_API_KEY, '');
const PINATA_API_SECRET = normalizeEnvValue(env.PINATA_SECRET_KEY, '');
const PINATA_GATEWAY_URL = normalizeEnvValue(env.PINATA_GATEWAY_URL, 'https://gateway.pinata.cloud');
...
export class SelfHostedIPFSClient implements IPFSAdapter {
  ...
  constructor(
    ...
    logger.info('Adaptador IPFS autoalojado inicializado');
  }
...
function getIPFSAdapter(): IPFSAdapter {
  ...
  logger.info('Usando proveedor de nodo IPFS autoalojado');
