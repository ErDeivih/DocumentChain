...
export class PinataIPFSClient implements IPFSAdapter {
  ...
  constructor(config: PinataConfig) {
    ...
    logger.info('Pinata IPFS adapter initialized', { gateway: this.gatewayUrl });
  }
  ...
  async pin(cid: string): Promise<void> {
    logger.debug(`Pinata pin called for ${cid} (already pinned on upload)`);
  }
  ...
  async getPinStatus(cid: string): Promise<any> {
    try {
      ...
      logger.error(`Error checking Pinata pin status for ${cid}:`, error);
      return {
        cid,
        status: 'unknown',
        peer_map: {}
  ...
  async unpinAll(): Promise<number> {
    logger.info('Fetching all Pinata pins for bulk cleanup...');
    ...
    logger.info(`Pinata bulk cleanup complete. Removed ${removed}/${pins.length} pins.`);
    return removed;
  }
}
