 ...
 */
export type HashType = 'argon2id' | 'argon2i' | 'argon2d' | 'pbkdf2' | 'unknown';
  ...
  static async hash(password: string): Promise<string> {
    try {
      const hash = await argon2.hash(password, {
        ...
        raw: false, // Devolver string, no Buffer
      });
      ...
      throw new ServiceUnavailableError('Error al hashear contraseña');
    }
  }
