...
export class DocumentTimelineService {
  ...
  static async getDocumentTimeline(
    ...
    if (document.blockchainId) {
      const results = await Promise.all(
        walletAddresses.map(async (address) => {
          try {
            return await BlockchainQueries.canRead(document.blockchainId!, address);
          } catch {
            return false;
          }
        })
      );
      canRead = results.some(Boolean);
    }
    if (!canRead) {
    ...
    if (!canRead) {
      throw new UnauthorizedError('No tienes permiso para ver este documento');
    }
