...
export async function getFolderById(req: Request, res: Response) {
  try {
    ...
    if (!folder) { res.status(404).json({ error: 'Carpeta no encontrada' }); return; }
    res.json({ folder });
  } catch (error) {
...
export async function createFolder(req: Request, res: Response) {
  try {
    ...
    if (!name || name.trim().length === 0) { res.status(400).json({ error: 'El nombre de la carpeta es requerido' }); return; }
    const folder = await FolderService.createFolder({ userId, name: name.trim(), description, parentId, color, icon });
...
export async function updateFolder(req: Request, res: Response) {
  try {
    ...
    const folder = await FolderService.updateFolder(req.params.id as string, userId, { name, description, color, icon, parentId });
    res.json({ folder });
  } catch (error) {
...
export async function moveDocumentsToFolder(req: Request, res: Response) {
  try {
    ...
    if (!Array.isArray(documentIds) || documentIds.length === 0) { res.status(400).json({ error: 'Se requiere un array de IDs de documentos' }); return; }
