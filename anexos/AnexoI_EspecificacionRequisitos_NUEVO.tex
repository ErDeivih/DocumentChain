...
Este anexo recoge la especificacion de requisitos del sistema DocumentChain. Se ha seguido la metodologia de Duran y Bernardez, que organiza la elicitacion en varios tipos de requisitos y establece un contrato claro entre quien desarrolla y quien evalua.

El sistema permite a los usuarios gestionar documentos de forma segura combinando varias tecnologias: cifrado para los documentos privados, almacenamiento en IPFS y registro inmutable en blockchain. Basicamente, lo que se busca es que cualquier persona pueda verificar si un documento es autentico sin necesidad de confiar en quien lo publico, por lo que la verificacion es totalmente independiente del proveedor.

La especificacion se organiza en tres bloques: requisitos de informacion, requisitos no funcionales y requisitos funcionales expresados como casos de uso. A continuacion se detalla cada uno de ellos.

\subsection*{Criterio de consolidación funcional del catálogo UC}

El catalogo funcional del proyecto se ha fijado en \textbf{39 casos de uso principales} (Tabla~\ref{tab:consolidacion-uc}). Durante el desarrollo se observo que muchas operaciones eran realmente pasos de otras mas grandes, asi que en vez de mantenerlas como casos de uso independientes se decidio agruparlas dentro de las que ya habia. La funcionalidad no se ha perdido: simplemente se ha reorganizado para que el catalogo sea mas manejable.

Por ejemplo, cosas como editar los metadatos de un documento o moverlo entre carpetas se consideran ahora parte de los casos de uso de Ver Detalle y Listar Documentos. Algunas operaciones de administracion tambien se han integrado en los casos de uso de gestion de usuarios y roles. La tabla de abajo muestra como ha quedado esta reorganizacion.
...
\begin{table}[H]
    ...
    \hline
    \textbf{Funcionalidad original} & \textbf{Descripción} & \textbf{Integrada en} \\
    \hline
    ...
    \hline
    \end{tabular}
    \caption{Consolidación del catálogo funcional en 39 casos de uso principales}
...
\begin{table}[H]
    ...
    \begin{itemize}
        ...
        \item Estado de verificación de email y material de recuperación de acceso
        \item Preferencias de notificación y configuración operativa de la cuenta
...
\begin{table}[H]
    ...
    \begin{itemize}
        ...
        \item Carpeta (FK), Etiquetas, Propietario (FK)
    ...
    \textbf{Document Share Key:}
    \begin{itemize}
        \item ID único, Documento (FK), Usuario destinatario (FK)
        \item Clave simétrica re-encriptada con la clave pública del destinatario
        \item Timestamp de creación
    \end{itemize} \\
    \hline
    \textbf{Importancia} & Vital \\
...
\subsubsection{IRQ-0003: Organización y etiquetado}
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{4cm}|p{10cm}|}
    \hline
    \textbf{IRQ-0003} & Organización y etiquetado \\
    \hline
    \textbf{Versión} & 1.0 \\
    \hline
    \textbf{Autores} & David Pérez Velasco \\
    ...
    \textbf{Fuentes} & A. Durán, B. Bernárdez \\
    \hline
    \textbf{Descripción} & Estructuras para organizar documentos mediante carpetas jerárquicas y etiquetas (tags) libres. \\
    \hline
    \textbf{Datos específicos} & 
    \textbf{Carpeta:}
    \begin{itemize}
    ...
    \textbf{Etiquetas (tags):}
    \begin{itemize}
        \item Array de cadenas de texto libre asociadas al documento
        \item Máximo 20 etiquetas por documento
        \item Longitud máxima 50 caracteres por etiqueta
    \end{itemize} \\
    \hline
    \textbf{Importancia} & Alta \\
    \hline
    ...
    \textbf{Estabilidad} & Alta \\
    \hline
    \textbf{Comentarios} & Carpetas soportan jerarquía multinivel. Las etiquetas son texto libre sin restricciones de color ni icono. \\
    \hline
    \end{tabular}
...
\begin{table}[H]
    ...
    \begin{itemize}
        ...
        \item Último bloque sincronizado
    \end{itemize} \\
    
    \textbf{Nota:} Las métricas de usuario (documentos totales, tamaño, firmas, etc.) se computan dinámicamente sobre las tablas operativas. No se mantiene una tabla de estadísticas desnormalizada por usuario. \\
    \hline
...
\begin{table}[H]
    ...
    \begin{enumerate}
        ...
        \item Integración con backend IPFS configurable (nodo Kubo autoalojado o Pinata), con soporte de redundancia según el entorno
...
\begin{table}[H]
    ...
    \begin{enumerate}
        ...
        \item Backend IPFS configurable con persistencia y estrategia de replicación según proveedor y entorno
...
\begin{table}[H]
    ...
    \begin{enumerate}
        ...
        \item Crear y gestionar carpetas
        ...
        \item Eliminar la cuenta desde la zona de peligro cuando resulte necesario
    \end{enumerate} \\
    \hline
...
\subsubsection{ACT-04: Servicio IPFS}
...
\begin{table}[H]
    ...
    \textbf{Fuentes} & A. Durán, B. Bernárdez \\
    \hline
    \textbf{Descripción} & Servicio de almacenamiento IPFS consumido por el backend mediante adaptador, con soporte para nodo Kubo autoalojado o proveedor gestionado (Pinata). \\
    \hline
    \textbf{Responsabilidades} & 
    \begin{enumerate}
    ...
    \end{enumerate} \\
    \hline
    \textbf{Comentarios} & Actor no humano. El backend interactúa vía API HTTP del proveedor configurado (Kubo o Pinata). \\
    \hline
    \end{tabular}
...
A continuacion se presentan los diagramas de casos de uso del sistema, comenzando por una vista general y detallando cada paquete funcional.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{diagramas/uc_acceso.png}
    \caption{Diagrama de casos de uso: Gestion de Acceso y Cuenta (UC-0001 a UC-0012)}
    \label{fig:uc-acceso}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{diagramas/uc_documentos.png}
    \caption{Diagrama de casos de uso: Gestion de Documentos (UC-0013 a UC-0021)}
    \label{fig:uc-documentos}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.90\linewidth]{diagramas/uc_versiones.png}
    \caption{Diagrama de casos de uso: Gestion de Versiones (UC-0022 a UC-0026)}
    \label{fig:uc-versiones}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{diagramas/uc_firmas.png}
    \caption{Diagrama de casos de uso: Gestion de Firmas (UC-0027 a UC-0029)}
    \label{fig:uc-firmas}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.90\linewidth]{diagramas/uc_comparticion.png}
    \caption{Diagrama de casos de uso: Gestion de Comparticion (UC-0030 a UC-0032)}
    \label{fig:uc-comparticion}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{diagramas/uc_auditoria.png}
    \caption{Diagrama de casos de uso: Auditoria, Verificacion y Estadisticas (UC-0033 a UC-0037)}
    \label{fig:uc-auditoria}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{diagramas/uc_administracion.png}
    \caption{Diagrama de casos de uso: Administracion (UC-0038 a UC-0039)}
    \label{fig:uc-administracion}
\end{figure}
...
\hline
\textbf{Descripción} & Permite a un usuario registrado y con correo verificado autenticarse en el sistema mediante credenciales de email o username y contraseña. \\
\hline
\textbf{Precondición} & El usuario debe estar registrado en el sistema y tener el correo ya verificado. \\
\hline
\textbf{Secuencia normal} &
...
\textbf{Paso 3:} El sistema valida las credenciales y comprueba la verificación previa del correo. \newline
\textbf{Paso 4:} El sistema emite JWT y token de refresco. \newline
\textbf{Paso 5:} El sistema registra la sesión activa y redirige al usuario a \texttt{/app/documents}. \\
\hline
...
\ucsubsubsection{UC-0003: Cerrar Sesión}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0003} & Cerrar Sesión \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\ucsubsubsection{UC-0004: Conectar Wallet}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0004} & Conectar Wallet \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\ucsubsubsection{UC-0005: Eliminar Wallet}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0005} & Eliminar Wallet \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0001, IRQ-0001, NFR-0001, UC-0004 \\
\hline
...
\ucsubsubsection{UC-0006: Establecer Wallet Principal}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0006} & Establecer Wallet Principal \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0001, OBJ-0002, IRQ-0001, NFR-0003, UC-0004 \\
\hline
...
\ucsubsubsection{UC-0007: Renombrar Wallet}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0007} & Renombrar Wallet \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0001, IRQ-0001, NFR-0003, UC-0004 \\
\hline
...
\ucsubsubsection{UC-0008: Gestionar Perfil y Notificaciones}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0008} & Gestionar Perfil y Notificaciones \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Estabilidad} & Alta \\
\hline
\textbf{Comentarios} & El username admite únicamente caracteres alfanuméricos, guiones y guiones bajos. El avatar se redimensiona automáticamente a 200x200 px en el backend. \\
\hline
\caption{Especificación del caso de uso UC-0008}
\end{longtable}

\ucsubsubsection{UC-0009: Eliminar Cuenta}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0009} & Eliminar Cuenta \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0001, IRQ-0001, IRQ-0002, NFR-0001, UC-0002 \\
\hline
\textbf{Descripción} & Permite al usuario eliminar permanentemente su cuenta desde la zona de peligro de configuración, suprimiendo sus datos de aplicación y cerrando su sesión activa. \\
\hline
\textbf{Precondición} & Usuario autenticado con JWT válido y acceso a \texttt{/app/settings}. \\
\hline
\textbf{Secuencia normal} & \textbf{Paso 1:} El usuario accede a la zona de peligro dentro de \texttt{Seguridad y Cuenta}.\newline\textbf{Paso 2:} Selecciona \texttt{Eliminar mi cuenta}.\newline\textbf{Paso 3:} El sistema muestra un modal de confirmación indicando el carácter irreversible de la acción.\newline\textbf{Paso 4:} El usuario confirma la eliminación.\newline\textbf{Paso 5:} El frontend envía \texttt{DELETE /users/me}.\newline\textbf{Paso 6:} El backend elimina la cuenta y los datos asociados de la aplicación, cierra la sesión y confirma la operación.\newline\textbf{Paso 7:} El frontend redirige al usuario a la página pública de inicio. \\
\hline
\textbf{Postcondición} & Cuenta eliminada de la aplicación. La sesión queda invalidada y el usuario deja de poder acceder a los recursos protegidos. \\
\hline
\textbf{Excepciones} & \textbf{Paso 4:} El usuario cancela la confirmación $\rightarrow$ No se ejecuta ningún cambio.\newline\textbf{Paso 5:} Sesión expirada $\rightarrow$ Error 401 y redirección al login.\newline\textbf{Paso 6:} Error interno durante la eliminación $\rightarrow$ El sistema informa de la incidencia y la cuenta permanece activa. \\
\hline
\textbf{Importancia} & Media \\
\hline
\textbf{Urgencia} & Baja \\
...
\textbf{Estabilidad} & Alta \\
\hline
\textbf{Comentarios} & La interfaz presenta esta operación como una acción destructiva y la aísla del resto de ajustes para reducir errores de uso. \\
\hline
\caption{Especificación del caso de uso UC-0009}
\end{longtable}

\ucsubsubsection{UC-0010: Cambiar Contraseña}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0010} & Cambiar Contraseña \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\ucsubsubsection{UC-0011: Verificar Email}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0011} & Verificar Email \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\hline
\textbf{Precondición} & El usuario debe haber recibido el email de verificación (enviado automáticamente en UC-0001 o al cambiar el email en UC-0008). El token de verificación debe estar vigente. \\
\hline
\textbf{Secuencia normal} &
...
\ucsubsubsection{UC-0012: Reenviar Verificaci\'{o}n de Email}
...
\hline
\multicolumn{2}{|c|}{\textbf{UC-0012: Reenviar Verificacion de Email}} \\
\hline
\textbf{Descripcion} & El usuario solicita el reenvio del correo de verificacion de email cuando el enlace original ha expirado o no se ha recibido. \\
\hline
\textbf{Actores} & ACT-01 (Usuario), ACT-05 (Sistema de Email) \\
\hline
\textbf{Precondiciones} & El usuario esta registrado pero su email no ha sido verificado. Existe un token de verificacion previo que puede estar caducado. \\
\hline
\textbf{Secuencia normal} &
\begin{enumerate}[nosep,leftmargin=*]
    \item El usuario accede a la pagina de inicio de sesion o a la vista de verificacion.
    \item El usuario pulsa el enlace ``Reenviar verificacion''.
    \item El sistema solicita la direccion de email asociada a la cuenta.
    \item El backend localiza al usuario por email y verifica que su cuenta no esta verificada.
    \item El sistema invalida cualquier token de verificacion previo no utilizado.
    \item El sistema genera un nuevo token de verificacion con fecha de expiracion.
    \item El sistema envia un nuevo email con el enlace de verificacion.
    \item El sistema muestra un mensaje de confirmacion indicando que el email ha sido reenviado.
\end{enumerate} \\
\hline
\textbf{Postcondiciones} & Existe un nuevo token de verificacion activo en el sistema. El token anterior ha sido invalidado. El usuario ha recibido un nuevo email de verificacion. \\
\hline
\textbf{Excepciones} & Email ya verificado (200 con mensaje informativo), usuario no encontrado (200 con exito para no revelar existencia), email invalido (400). \\
\hline
\end{tabular}
\caption{Especificacion del caso de uso UC-0012}
\end{table}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{diagramas/seq-bce-uc0012-resend-verification.png}
    \caption{Diagrama de secuencia de analisis BCE -- UC-0012: Reenviar Verificacion de Email}
    \label{fig:seq-bce-uc0012-resend}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{diagramas/seq-resend-verification.png}
    \caption{Diagrama de secuencia de diseno -- UC-0012: Reenviar Verificacion de Email}
    \label{fig:seq-uc0012-resend}
\end{figure}

\ucsubsubsection{UC-0013: Subir Documento}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0013} & Subir Documento \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Paso 3:} El backend prepara la transacción, genera un mensaje legible con el título del documento y la dirección del contrato, y solicita la firma al usuario mediante su wallet.\newline
\textbf{Paso 4:} El usuario confirma el mensaje legible en la wallet y el frontend envía la firma al backend para completar el registro on-chain.\newline
\textbf{Paso 5:} Una vez confirmada la transacción, el documento queda disponible en la biblioteca del usuario y, si es público, se habilita su acceso mediante enlace y código QR.\\
\hline
...
\textbf{Paso 3:} El usuario rechaza la firma en la wallet $\rightarrow$ Operación cancelada.\newline
\textbf{Paso 4:} El backend rechaza la firma o el mensaje no coincide $\rightarrow$ Error 400.\newline
\textbf{Paso 5:} La transacción revierte o no se confirma on-chain $\rightarrow$ El documento no pasa a estado operativo.\\
\hline
\textbf{Importancia} & Vital \\
\hline
\textbf{Urgencia} & Inmediata \\
...
\textbf{Estabilidad} & Alta \\
\hline
\textbf{Comentarios} & El patrón Prepare/Confirm evita inconsistencias entre IPFS, base de datos y blockchain. El backend genera un mensaje legible (\textit{messageToSign}) para que el usuario verifique en la wallet qué documento está creando antes de firmar. La publicación pública es siempre optativa y exige una advertencia explícita porque elimina el cifrado del contenido. \\
\hline
\caption{Especificación del caso de uso UC-0013}
\end{longtable}

\ucsubsubsection{UC-0014: Listar Documentos}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0014} & Listar Documentos \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\hline
\textbf{Descripci\'{o}n} & Permite al usuario visualizar todos sus documentos con capacidades de filtrado y ordenacion. \\
\hline
\textbf{Precondici\'{o}n} & El usuario debe haber iniciado sesi\'{o}n. \\
\hline
\textbf{Secuencia normal} &
\textbf{Paso 1:} El usuario accede a la vista principal de documentos.\newline
\textbf{Paso 2:} El sistema ejecuta \texttt{GET /documents} con filtros opcionales.\newline
\textbf{Paso 3:} El backend consulta la base de datos aplicando los filtros solicitados.\newline
\textbf{Paso 4:} El sistema devuelve la lista paginada de documentos.\\
\hline
\textbf{Postcondici\'{o}n} & Lista de documentos propia del usuario visible con informaci\'{o}n resumida de cada uno. \\
\hline
\textbf{Excepciones} &
\textbf{Paso 2:} Error de autenticaci\'{o}n $\rightarrow$ Redirigir al inicio de sesi\'{o}n.\\
\hline
\textbf{Importancia} & Alta \\
\hline
\textbf{Urgencia} & Alta \\
...
\textbf{Estabilidad} & Alta \\
\hline
\textbf{Comentarios} & La lista de esta vista contiene unicamente documentos propios. Los documentos compartidos con el usuario se muestran en una vista separada (UC-0032). \\
\hline
\caption{Especificaci\'{o}n del caso de uso UC-0014}
\end{longtable}

\ucsubsubsection{UC-0015: Ver Detalle de Documento}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0015} & Ver Detalle de Documento \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0002, OBJ-0003, IRQ-0002, IRQ-0003, IRQ-0005, NFR-0003, UC-0011 \\
\hline
...
\ucsubsubsection{UC-0016: Descargar Documento}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0016} & Descargar Documento \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0002, OBJ-0003, IRQ-0002, IRQ-0003, NFR-0001, NFR-0002, UC-0012 \\
\hline
...
\ucsubsubsection{UC-0017: Archivar Documento}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0017} & Archivar Documento \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0003, IRQ-0002, NFR-0003, UC-0012 \\
\hline
...
\ucsubsubsection{UC-0018: Desarchivar Documento}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0018} & Desarchivar Documento \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0002, OBJ-0003, IRQ-0002, NFR-0003, UC-0017 \\
\hline
...
\textbf{Estabilidad} & Alta \\
\hline
\textbf{Comentarios} & Operación inversa a UC-0017 (Archivar Documento). \\
\hline
\caption{Especificación del caso de uso UC-0018}
\end{longtable}

\ucsubsubsection{UC-0019: Eliminar Documento}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0019} & Eliminar Documento \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0002, OBJ-0003, IRQ-0002, NFR-0001, NFR-0005, UC-0012 \\
\hline
...
\ucsubsubsection{UC-0020: Transferir Documento}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0020} & Transferir Documento \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0002, OBJ-0003, OBJ-0006, IRQ-0002, IRQ-0005, NFR-0001, NFR-0002, UC-0013 \\
\hline
...
\ucsubsubsection{UC-0021: Buscar Documentos}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0021} & Buscar Documentos \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0003, IRQ-0002, NFR-0003, UC-0011 \\
\hline
    \textbf{Descripción} & Permite al usuario buscar documentos propios y compartidos aplicando múltiples filtros simultáneos: texto libre en nombre y descripción, tipo de archivo, carpeta, estado blockchain, firmante, etiquetas y rango de fechas. \\
\hline
\textbf{Precondición} & Usuario autenticado con JWT válido. \\
\hline
\textbf{Secuencia normal} &
...
\textbf{Paso 3:} El frontend envía con los parámetros: q (texto libre), type (tipo MIME), folderId, blockchainStatus, signedBy, tags, dateFrom, dateTo, page, limit.\newline
...
\ucsubsubsection{UC-0022: Crear Nueva Versión}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0022} & Crear Nueva Versión \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0002, OBJ-0003, IRQ-0002, IRQ-0003, NFR-0001, NFR-0002, UC-0013, UC-0012 \\
\hline
...
\textbf{Paso 3:} El backend registra la nueva versión en el almacenamiento configurado, prepara la transacción blockchain y genera un mensaje legible con el título del documento y el número de versión.\newline
\textbf{Paso 4:} El usuario revisa el mensaje legible en su wallet y confirma la firma; el frontend envía la firma al backend para completar el registro on-chain.\newline
\textbf{Paso 5:} Tras la confirmación de la transacción, la nueva versión pasa a formar parte del historial y se convierte automáticamente en la versión operativa.\\
\hline
...
\textbf{Paso 4:} El usuario rechaza la firma $\rightarrow$ Operación cancelada.\newline
\textbf{Paso 4:} El backend rechaza la firma o el mensaje no coincide $\rightarrow$ Error 400.\newline
\textbf{Paso 5:} La transacción revierte o no se confirma $\rightarrow$ La versión no pasa a estado operativo.\\
\hline
\textbf{Importancia} & Alta \\
\hline
\textbf{Urgencia} & Alta \\
...
\ucsubsubsection{UC-0024: Establecer Versión Operativa}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0024} & Establecer Versión Operativa \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0003, IRQ-0002, UC-0023 \\
\hline
...
\ucsubsubsection{UC-0025: Restaurar Versión Anterior}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0025} & Restaurar Versión Anterior \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0003, IRQ-0002, UC-0023, UC-0024 \\
\hline
...
\textbf{Paso 6:} El sistema ejecuta UC-0024 para establecer la versión seleccionada como operativa.\newline
...
\ucsubsubsection{UC-0026: Descargar Versión Específica}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0026} & Descargar Versión Específica \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0002, OBJ-0003, IRQ-0002, NFR-0001, UC-0016, UC-0023 \\
\hline
\textbf{Descripción} & Permite al usuario con acceso descargar una versión concreta del documento (no necesariamente la operativa). El flujo sigue la rama privada o pública descrita en UC-0016 según la visibilidad del documento. \\
\hline
...
\ucsubsubsection{UC-0027: Firmar Documento}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0027} & Firmar Documento \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Paso 3:} El backend prepara la operación, genera un mensaje legible que incluye el título del documento, el número de versión y la dirección del contrato, y devuelve los datos necesarios para la firma.\newline
\textbf{Paso 4:} El usuario revisa el mensaje legible en su wallet y confirma la firma.\newline
\textbf{Paso 5:} El frontend envía la firma al backend, que completa el registro on-chain.\newline
\textbf{Paso 6:} La blockchain registra la firma y el sistema sincroniza su estado operativo.\newline
\textbf{Paso 7:} El propietario recibe la notificación correspondiente y la interfaz actualiza el listado de firmas.\\
\hline
...
\textbf{Paso 5:} El backend rechaza la firma o el mensaje no coincide $\rightarrow$ Error 400.\newline
\textbf{Paso 6:} La transacción revierte $\rightarrow$ Estado FAILED, firma no registrada.\\
\hline
\textbf{Importancia} & Alta \\
\hline
\textbf{Urgencia} & Alta \\
...
\textbf{Estabilidad} & Alta \\
\hline
\textbf{Comentarios} & Proporciona no repudio legal. La firma incluye hash de contenido. El backend genera un mensaje legible (\textit{messageToSign}) para que el usuario pueda verificar en la wallet qué documento está firmando antes de confirmar. \\
\hline
\caption{Especificación del caso de uso UC-0027}
\end{longtable}

\ucsubsubsection{UC-0028: Ver Firmas de un Documento}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0028} & Ver Firmas de un Documento \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0002, OBJ-0004, IRQ-0004, NFR-0001, UC-0012 \\
\hline
...
\hline
\textbf{Postcondición} & Lista de firmas visible para la versión consultada. El usuario puede identificar al firmante y verificar la evidencia blockchain asociada mediante UC-0029. \\
\hline
\textbf{Excepciones} &
...
\ucsubsubsection{UC-0029: Verificar Firma en Blockchain}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0029} & Verificar Firma del Usuario \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0002, OBJ-0004, IRQ-0004, NFR-0001, NFR-0002, UC-0028 \\
\hline
...
\ucsubsubsection{UC-0030: Compartir Documento}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0030} & Compartir Documento \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\ucsubsubsection{UC-0031: Revocar Acceso a Documento}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0031} & Revocar Acceso a Documento \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0002, OBJ-0004, IRQ-0005, NFR-0001, UC-0030 \\
\hline
...
\ucsubsubsection{UC-0032: Ver Documentos Compartidos Conmigo}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0032} & Ver Documentos Compartidos Conmigo \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Estabilidad} & Alta \\
\hline
\textbf{Comentarios} & Los documentos compartidos también aparecen en el listado general (UC-0014), pero esta vista filtrada mejora la organización y permite al usuario distinguir claramente entre documentos propios y recibidos. \\
\hline
\caption{Especificación del caso de uso UC-0032}
\end{longtable}

\ucsubsubsection{UC-0033: Ver Timeline de Documento}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0033} & Ver Timeline de Documento \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0003, OBJ-0004, IRQ-0007, NFR-0001, UC-0012 \\
\hline
...
\textbf{Paso 7:} El frontend renderiza la línea de tiempo con iconos diferenciados por tipo de evento y colores según tipo de evento.\newline
...
\ucsubsubsection{UC-0034: Ver Estadísticas Personales}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0034} & Ver Estadísticas Personales \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0001, OBJ-0003, IRQ-0007, NFR-0001, UC-0002, UC-0039 \\
\hline
...
\ucsubsubsection{UC-0036: Verificar Autenticidad de Documento}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0036} & Verificar Autenticidad de Documento \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\ucsubsubsection{UC-0037: Auditoría Blockchain}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0037} & Auditoría Blockchain \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0002, OBJ-0004, IRQ-0007, NFR-0001, NFR-0002, UC-0035 \\
\hline
...
\ucsubsubsection{UC-0038: Listar Usuarios}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0038} & Listar Usuarios \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0001, IRQ-0001, NFR-0001, UC-0039 \\
\hline
...
\ucsubsubsection{UC-0039: Gestionar Roles de Usuario}

\begin{longtable}{|p{4cm}|p{10cm}|}
\hline
\textbf{UC-0039} & Gestionar Roles de Usuario \\
\hline
\textbf{Versión} & 1.0 \\
\hline
\textbf{Autores} & David Pérez Velasco \\
...
\textbf{Fuentes} & A. Durán, B. Bernárdez \\
\hline
\textbf{Dependencias} & OBJ-0001, IRQ-0001, NFR-0001, UC-0038 \\
\hline
...
La matriz UC-UC agrupa los 39 casos de uso principales en siete paquetes funcionales y muestra las dependencias de ejecución entre ellos. Una X indica que el paquete de la fila requiere la funcionalidad del paquete de la columna para su correcto funcionamiento.
...
\subsection{Matriz de rastreabilidad UC -- Diagramas de Secuencia}

La Tabla~\ref{tab:matriz-uc-seq} cruza cada caso de uso con los diagramas de secuencia de analisis (BCE) y de diseno que lo documentan en el Anexo III.

\begin{center}
\small
\renewcommand{\arraystretch}{1.1}
\begin{longtable}{|p{2.8cm}|c|c|}
\hline
\textbf{Caso de Uso} & \textbf{Seq. Analisis (BCE)} & \textbf{Seq. Diseno} \\
\hline
\endfirsthead
\hline
\textbf{Caso de Uso} & \textbf{Seq. Analisis (BCE)} & \textbf{Seq. Diseno} \\
...
\hline
UC-0002 Iniciar Sesion & seq-bce-uc0002-login.png & seq-login.png \\
\hline
UC-0003 Cerrar Sesion & seq-bce-uc0003-logout.png & seq-logout.png \\
\hline
...
\hline
UC-0009 Eliminar Cuenta & seq-bce-uc0009-delete-account.png & seq-bce-uc0009-delete-account.png \\
\hline
UC-0010 Cambiar Contrasena & seq-bce-uc0010-change-password.png & seq-change-password.png \\
\hline
UC-0011 Verificar Email & seq-bce-uc0011-verify-email-single.png & seq-verify-email.png \\
\hline
UC-0012 Reenviar Verificacion de Email & seq-bce-uc0012-resend-verification.png & seq-resend-verification.png \\
\hline
UC-0013 Subir Documento & seq-bce-uc0013-upload.png & seq-upload.png \\
\hline
UC-0014 Listar Documentos & seq-bce-uc0014-list.png & seq-list-documents.png \\
\hline
UC-0015 Ver Detalle de Documento & seq-bce-uc0015-detail.png & seq-doc-detail.png \\
\hline
UC-0016 Descargar Documento & seq-bce-uc0016-download.png & seq-download.png \\
\hline
UC-0017 Archivar Documento & seq-bce-uc0017-archive.png & seq-archive.png \\
\hline
UC-0018 Desarchivar Documento & seq-bce-uc0018-unarchive.png & seq-unarchive.png \\
\hline
UC-0019 Eliminar Documento & seq-bce-uc0019-delete.png & seq-delete.png \\
\hline
UC-0020 Transferir Documento & seq-bce-uc0020-transfer.png & seq-transfer.png \\
\hline
UC-0021 Buscar Documentos & seq-bce-uc0021-search.png & seq-search.png \\
\hline
UC-0022 Crear Nueva Version & seq-bce-uc0022-version.png & seq-version.png \\
\hline
UC-0023 Listar Versiones & seq-bce-uc0023-list-versions.png & seq-list-versions.png \\
\hline
UC-0024 Establecer Version Operativa & seq-bce-uc0024-operational.png & seq-activate-version.png \\
\hline
UC-0025 Restaurar Version Anterior & seq-bce-uc0025-restore.png & seq-restore-version.png \\
\hline
UC-0026 Descargar Version Especifica & seq-bce-uc0026-download-version.png & seq-download-version.png \\
\hline
UC-0027 Firmar Documento & seq-bce-uc0027-sign.png & seq-sign.png \\
\hline
UC-0028 Ver Firmas de un Documento & seq-bce-uc0028-view-signatures.png & seq-view-signatures.png \\
\hline
UC-0029 Verificar Firma del Usuario & seq-bce-uc0029-verify-signature.png & seq-verify-signature.png \\
\hline
UC-0030 Compartir Documento & seq-bce-uc0030-share.png & seq-share.png \\
\hline
UC-0031 Revocar Acceso a Documento & seq-bce-uc0031-revoke.png & seq-revoke-access.png \\
\hline
UC-0032 Ver Compartidos Conmigo & seq-bce-uc0032-shared-with-me.png & seq-shared-documents.png \\
\hline
UC-0033 Ver Timeline de Documento & seq-bce-uc0033-timeline.png & seq-timeline.png \\
\hline
UC-0034 Ver Estadisticas Personales & seq-bce-uc0034-stats.png & seq-user-stats.png \\
\hline
UC-0035 Ver Estadisticas Globales & seq-bce-uc0035-global-stats.png & seq-global-stats.png \\
\hline
UC-0036 Verificar Autenticidad & seq-bce-uc0036-verify-authenticity.png & seq-verify-authenticity.png \\
\hline
UC-0037 Auditoria Blockchain & seq-bce-uc0037-audit.png & seq-blockchain-audit.png \\
\hline
UC-0038 Listar Usuarios & seq-bce-uc0038-list-users.png & seq-list-users.png \\
\hline
UC-0039 Gestionar Roles de Usuario & seq-bce-uc0039-manage-roles.png & seq-manage-roles.png \\
\hline
\caption{Matriz de rastreabilidad: Casos de Uso -- Diagramas de Secuencia}
\label{tab:matriz-uc-seq}
\end{longtable}
\end{center}

\section{Matriz de Rastreabilidad Global}

La Tabla~\ref{tab:mtr-global} presenta la matriz de rastreabilidad consolidada que cruza los requisitos de informacion (IRQ) y los requisitos no funcionales (NFR) con los 39 casos de uso del sistema, indicando tambien los diagramas de secuencia de analisis y diseno asociados a cada caso de uso en el Anexo III.

{\setlength{\tabcolsep}{2pt}
\begin{center}
{\tiny
\begin{longtable}{|p{1.5cm}|p{1.0cm}p{0.9cm}p{0.9cm}p{0.9cm}p{0.9cm}p{0.9cm}p{0.9cm}|p{1.5cm}|}
\caption{Matriz de rastreabilidad global IRQ/NFR -- UC -- Diagramas de secuencia} \label{tab:mtr-global} \\
\hline
\textbf{Caso de Uso} & \multicolumn{7}{c|}{\textbf{IRQ}} & \textbf{Diagramas} \\
\hline
\textbf{UC ID} & \textbf{1} & \textbf{2} & \textbf{3} & \textbf{4} & \textbf{5} & \textbf{6} & \textbf{7} & \textbf{Anexo III} \\
\hline
\hline
\endhead
\multicolumn{9}{c}{\textbf{Modulo de Acceso y Cuenta (UC-0001 a UC-0012)}} \\
\hline
UC-0001 Registrar Usuario & X &  &  &  &  &  &  & BCE + Diseno \\
\hline
UC-0002 Iniciar Sesion & X &  &  &  &  &  &  & BCE + Diseno \\
\hline
UC-0003 Cerrar Sesion & X &  &  &  &  &  &  & BCE + Diseno \\
\hline
UC-0004 Conectar Wallet & X &  &  &  &  &  &  & BCE + Diseno \\
\hline
UC-0005 Eliminar Wallet & X &  &  &  &  &  &  & BCE + Diseno \\
\hline
UC-0006 Establecer Wallet Principal & X &  &  &  &  &  &  & BCE + Diseno \\
\hline
UC-0007 Renombrar Wallet & X &  &  &  &  &  &  & BCE + Diseno \\
\hline
UC-0008 Gestionar Perfil y Notif. & X &  &  &  &  &  & X & BCE + Diseno \\
\hline
UC-0009 Eliminar Cuenta & X &  &  &  &  &  &  & BCE + Diseno \\
\hline
UC-0010 Cambiar Contrasena & X &  &  &  &  &  &  & BCE + Diseno \\
\hline
UC-0011 Verificar Email & X &  &  &  &  &  &  & BCE + Diseno \\
\hline
UC-0012 Reenviar Verificacion & X &  &  &  &  &  &  & BCE + Diseno \\
\hline
\multicolumn{9}{c}{\textbf{Modulo de Gestion de Documentos (UC-0013 a UC-0021)}} \\
\hline
UC-0013 Subir Documento & X & X & X &  &  &  &  & BCE + Diseno \\
\hline
UC-0014 Listar Documentos &  & X & X &  &  &  &  & BCE + Diseno \\
\hline
UC-0015 Ver Detalle de Documento &  & X & X &  &  &  &  & BCE + Diseno \\
\hline
UC-0016 Descargar Documento & X & X & X &  &  &  &  & BCE + Diseno \\
\hline
UC-0017 Archivar Documento &  & X &  &  &  &  &  & BCE + Diseno \\
\hline
UC-0018 Desarchivar Documento &  & X &  &  &  &  &  & BCE + Diseno \\
\hline
UC-0019 Eliminar Documento &  & X &  &  &  &  &  & BCE + Diseno \\
\hline
UC-0020 Transferir Documento &  & X &  &  & X &  &  & BCE + Diseno \\
\hline
UC-0021 Buscar Documentos &  & X &  &  &  &  &  & BCE + Diseno \\
\hline
\multicolumn{9}{c}{\textbf{Modulo de Versionado (UC-0022 a UC-0026)}} \\
\hline
UC-0022 Crear Nueva Version &  & X & X &  &  &  &  & BCE + Diseno \\
\hline
UC-0023 Listar Versiones &  & X &  &  &  &  &  & BCE + Diseno \\
\hline
UC-0024 Establecer Version Operativa &  & X &  &  &  &  &  & BCE + Diseno \\
\hline
UC-0025 Restaurar Version Anterior &  & X &  &  &  &  &  & BCE + Diseno \\
\hline
UC-0026 Descargar Version Especifica &  & X &  &  &  &  &  & BCE + Diseno \\
\hline
\multicolumn{9}{c}{\textbf{Modulo de Firmas Digitales (UC-0027 a UC-0029)}} \\
\hline
UC-0027 Firmar Documento &  &  &  & X &  &  &  & BCE + Diseno \\
\hline
UC-0028 Ver Firmas de un Documento &  &  &  & X &  &  &  & BCE + Diseno \\
\hline
UC-0029 Verificar Firma del Usuario &  &  &  & X &  &  &  & BCE + Diseno \\
\hline
\multicolumn{9}{c}{\textbf{Modulo de Comparticion (UC-0030 a UC-0032)}} \\
\hline
UC-0030 Compartir Documento &  &  &  &  & X &  &  & BCE + Diseno \\
\hline
UC-0031 Revocar Acceso &  &  &  &  & X &  &  & BCE + Diseno \\
\hline
UC-0032 Ver Compartidos Conmigo &  &  &  &  & X &  &  & BCE + Diseno \\
\hline
\multicolumn{9}{c}{\textbf{Modulo de Auditoria y Verificacion (UC-0033 a UC-0037)}} \\
\hline
UC-0033 Ver Timeline &  &  &  &  &  &  & X & BCE + Diseno \\
\hline
UC-0034 Ver Estadisticas Personales &  &  &  &  &  & X & X & BCE + Diseno \\
\hline
UC-0035 Ver Estadisticas Globales &  &  &  &  &  & X & X & BCE + Diseno \\
\hline
UC-0036 Verificar Autenticidad &  & X &  &  &  &  & X & BCE + Diseno \\
\hline
UC-0037 Auditoria Blockchain &  &  &  &  &  &  & X & BCE + Diseno \\
\hline
\multicolumn{9}{c}{\textbf{Modulo de Administracion (UC-0038 a UC-0039)}} \\
\hline
UC-0038 Listar Usuarios & X &  &  &  &  &  &  & BCE + Diseno \\
\hline
UC-0039 Gestionar Roles & X &  &  &  &  &  &  & BCE + Diseno \\
\hline
\end{longtable}
}
\end{center}

\noindent La tabla anterior muestra la cobertura completa de los requisitos de informacion por parte de los 39 casos de uso. Todos los IRQ estan cubiertos por al menos un caso de uso, y cada caso de uso cuenta con diagramas de secuencia de analisis (BCE) y diseno documentados en el Anexo III.

\section{Referencias}
\addcontentsline{toc}{section}{Referencias}
\begin{enumerate}
  \item Durán, A. y Bernárdez, B. (2001). \textit{Metodología de elicitación de requisitos software}. Universidad de Sevilla.
  \item OpenZeppelin. (2024). \textit{OpenZeppelin Contracts}. https://docs.openzeppelin.com/contracts
  \item Protocol Labs. (2024). \textit{IPFS Documentation}. https://docs.ipfs.io
  \item Buterin, V. (2014). \textit{Ethereum White Paper}. https://ethereum.org
  \item Nakamoto, S. (2008). \textit{Bitcoin: A Peer-to-Peer Electronic Cash System}.
  \item Wood, G. (2014). \textit{Ethereum: A Secure Decentralised Generalised Transaction Ledger}. Ethereum Project Yellow Paper.
  \item Benet, J. (2014). \textit{IPFS - Content Addressed, Versioned, P2P File System}. arXiv:1407.3561.
\end{enumerate}
