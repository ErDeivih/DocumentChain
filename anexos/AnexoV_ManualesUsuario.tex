...
\hline
\texttt{/public/d/:publicId} & Página pública de un documento publicado con enlace compartible. & Pública \\
\hline
\texttt{/public/d/:publicId/v/:versionNumber} & Página pública de una versión concreta del documento publicado. & Pública \\
\hline
...
\hline
\texttt{/app/documents/:id/timeline} & Historial cronológico detallado del documento. & Autenticado (propietario/compartido) \\
\hline
...
\hline
\texttt{/app/settings} & Configuración de cuenta, seguridad, wallets y privacidad. & Autenticado \\
\hline
...
\begin{figure}[H]
    ...
    \caption{Portada pública de DocumentChain}
...
\begin{enumerate}
    ...
    \item ¡Listo! Ya puedes iniciar sesión.
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{Error de verificación}: el token no es válido, ha caducado o falta en la URL.
    \end{itemize}
    \item Desde esa misma página puedes volver a \textbf{Iniciar sesión} o regresar al formulario de \textbf{Registro}.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Si la cuenta tiene 2FA activo, se abrirá una segunda pantalla de verificación.
...
\textbf{Nota}: En la versión actual del sistema, las wallets \textbf{no} se utilizan para iniciar sesión ni para registrarse. Su uso queda reservado para firmar operaciones blockchain como subida, compartición, firmado o reactivación de cuenta.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{Categoría} (opcional): Selecciona una categoría
    ...
    \begin{figure}[H]
        ...
        \caption{Selector de wallet utilizado para firmar operaciones blockchain}
        \label{fig:wallet-selector-modal}
    \end{figure}
...
\begin{enumerate}
    ...
    \item La búsqueda encuentra coincidencias en:
    \begin{itemize}
        \item Nombre del documento
        \item Descripción
        \item Tags
        \item Nombre de carpeta/categoría
    \end{itemize}
...
\begin{enumerate}
    ...
    \begin{figure}[H]
        ...
        \caption{Vista de detalles del documento}
    ...
    \begin{itemize}
        ...
        \item Identificador blockchain del registro
...
\begin{enumerate}
    ...
    \item El sistema solicitará seleccionar una wallet y confirmar la transacción de firma.
    
    \item Espera a que la transacción se mine.
...
\textbf{Nota}: La firma se registra on-chain con tu dirección de wallet y el timestamp del bloque, proporcionando no-repudio criptográfico.
...
\begin{enumerate}
    ...
    \item En esa ficha se muestran el nombre visible, el alias disponible, la wallet utilizada para la firma, la fecha registrada y el estado de sincronización blockchain.
...
\begin{enumerate}
    ...
    \item Una vez confirmada, el usuario aparecerá en la lista de accesos compartidos del detalle.
...
\begin{enumerate}
    ...
    \item Cada tarjeta indica el propietario del documento y, cuando procede, la categoría asociada.
...
\begin{figure}[H]
    ...
    \caption{Vista de documentos compartidos con la wallet activa}
...
\begin{enumerate}
    ...
    \item Busca al nuevo propietario por nombre de usuario.
...
La clasificación complementaria se realiza durante la subida o la creación de nuevas versiones, donde es posible asociar \textbf{carpeta}, \textbf{categoría} y \textbf{etiquetas} para facilitar búsquedas posteriores.

\subsubsection{Clasificar con categorías y etiquetas}

\begin{enumerate}
    \item Durante la subida de un documento o la publicación de una nueva versión, utiliza los campos \textbf{Categoría} y \textbf{Etiquetas} del formulario.
    \item La categoría permite agrupar documentos por temática o tipo de expediente.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Transferencias
        \item Descargas (si el propietario activó tracking)
    \end{itemize}
\end{enumerate}
...
\begin{figure}[H]
    ...
    \caption{Pantalla de verificación autenticada de documentos}
...
\begin{enumerate}
    \item Abre la URL pública con formato \texttt{/public/d/:publicId} o una variante de versión \texttt{/public/d/:publicId/v/:versionNumber}.
...
\begin{enumerate}
    ...
    \item Ve a la pestaña \textbf{"Notificaciones"}.
...
\begin{figure}[H]
    ...
    \caption{Pantalla de configuración de cuenta}
...
\begin{enumerate}
    ...
    \item En la pestaña \textbf{"Perfil"}, puedes editar:
    \begin{itemize}
        \item Nombre completo
        \item Email (requiere re-verificación)
...
\begin{enumerate}
    \item Ve a la pestaña \textbf{"Seguridad"}.
    \item Introduce:
    \begin{itemize}
...
\begin{enumerate}
    \item En la pestaña \textbf{"Seguridad"}, pulsa \textbf{"Configurar 2FA"}.
...
\begin{figure}[H]
    ...
    \caption{Configuración de autenticación de dos factores desde Ajustes}
...
\begin{enumerate}
    \item Vuelve al bloque de \textbf{2FA} dentro de la pestaña \textbf{"Seguridad"}.
...
\subsubsection{Privacidad, exportación y suspensión temporal}

\begin{enumerate}
    \item En la pestaña \textbf{"Privacidad"}, puedes ajustar:
    \begin{itemize}
        \item La visibilidad de tu perfil
        \item La posibilidad de recibir documentos compartidos
        \item La solicitud de exportación de datos
    \end{itemize}
    \item En la \textbf{Zona de Peligro} puedes suspender temporalmente la cuenta o iniciar el proceso de eliminación.
    \item La suspensión voluntaria requiere firmar con la wallet principal para bloquear también la capacidad de operar en blockchain.
\end{enumerate}

\subsubsection{Reactivar una cuenta suspendida}

\begin{enumerate}
    \item Si la cuenta está suspendida, el sistema te mantendrá en \texttt{/app/settings} y dejará accesible la pestaña \textbf{"Privacidad"}.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Estado operativo de \textbf{base de datos}, \textbf{API}, \textbf{clúster IPFS}, \textbf{blockchain}, \textbf{servicio de email} y \textbf{backend de almacenamiento}
...
\begin{enumerate}
    \item Accede a \texttt{/app/blockchain} desde la barra lateral.
    
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.95\textwidth]{capturas-ui/blockchain-auditor.png}
        \caption{Herramienta de auditoría blockchain}
    ...
    \begin{itemize}
        ...
        \item Un listado expandible de \textbf{eventos recientes} con tipo, fecha, bloque, hash de transacción y metadatos asociados
