...
El proyecto tiene una complejidad particular porque mezcla cuatro tecnologías muy distintas entre sí: un \textit{backend} REST en Node.js, una base de datos PostgreSQL, almacenamiento descentralizado sobre IPFS mediante un proveedor IPFS configurable (Pinata/Kubo) y un \textit{smart contract} en Solidity sobre Ethereum. Tener que coordinar cuatro subsistemas tan dispares justifica tanto la elección del Proceso Unificado como el esfuerzo de estimación que se detalla a continuación.
...
El proyecto combina un frontend React para la operativa de usuario y el manejo del material criptográfico, un backend Node.js con lógica compleja (cifrado AES-256-GCM, encapsulado RSA-OAEP de claves, integración blockchain), almacenamiento documental mediante IPFS con Pinata como proveedor principal y un smart contract Solidity como registro inmutable de la custodia documental. Cada subsistema ha necesitado su propio análisis, diseño, implementación y pruebas.

La estimación de \textbf{1.400 horas} que arrojó EZEstimate ($411 \times 3{,}4$) ha resultado coherente con el esfuerzo real. Hay que entenderla como una referencia orientativa, porque durante el desarrollo surgieron tareas que no siempre estaban reflejadas en los casos de uso: investigar sobre blockchain, resolver problemas de sincronización de eventos, afinar la arquitectura de cifrado, pruebas continuas de seguridad, etc.
