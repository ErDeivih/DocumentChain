...
\textbf{Storyboard 1: Alta y firma de un documento por parte del propietario.} El usuario necesita registrar un contrato en la plataforma y dejar constancia de su firma. Accede a la plataforma con sus credenciales, activa su wallet si no lo ha hecho previamente y selecciona la opción de subir documento. Completa el formulario de alta con título, descripción, carpeta (opcional), etiquetas y opción de visibilidad. El backend prepara la transacción, calcula el hash del documento y devuelve un mensaje legible que incluye el título del documento, la versión y la dirección del contrato. La wallet muestra ese mensaje en lugar de datos crudos, y el usuario lo firma consciente de qué está suscribiendo. Una vez confirmada, el frontend envía la firma al backend para que complete el registro on-chain. El documento aparece en su listado con el CID de IPFS, el hash de la transacción y la opción de consultar la evidencia en el explorador blockchain. Desde el detalle del documento, el usuario accede a la pestaña de firmas y suscribe el documento con su wallet, añadiendo un comentario visible para los usuarios compartidos.
