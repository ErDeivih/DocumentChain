...
funcion prepararDocumento(hashContenido, cid, usuario):
    ...
    txData = contrato.populateTransaction('registerDocument', hash, cid)
...
Los smart contracts se han desarrollado en Solidity y desplegado mediante Hardhat. En las subsecciones siguientes se documentan los contratos individuales originales y las razones que motivaron su posterior consolidacion.

\subsection{DocumentRegistry.sol}

Registra el hash de contenido y los metadatos de cada documento en la blockchain. Aunque la implementacion final se consolido en un unico contrato, la logica de registro basica se ilustra en la List~\ref{lst:registry}.

\begin{lstlisting}[language=Java, caption={DocumentRegistry.sol -- Registro de documentos}, label={lst:registry}]
...
contract DocumentRegistry {
    ...
    function registerDocument(
        ...
        require(!documents[_contentHash].exists, "Documento ya registrado");
        ...
        emit DocumentCreated(_contentHash, _ipfsCid, msg.sender);
    }
}
\end{lstlisting}

\subsection{Versioning.sol}

Gestiona las versiones de los documentos registrados, permitiendo mantener un historial inmutable de cambios.

\begin{lstlisting}[language=Java, caption={Versioning.sol -- Gestion de versiones}]
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;
...
contract Versioning {
    ...
    mapping(bytes32 => Version[]) public documentVersions;
    mapping(bytes32 => uint256) public operationalVersion;
    ...
    function addVersion(
        ...
        uint256 newVersion = documentVersions[_docHash].length + 1;
        documentVersions[_docHash].push(Version({
            versionNumber: newVersion,
            contentHash: _contentHash,
            ipfsCid: _ipfsCid,
            timestamp: block.timestamp
        }));
        operationalVersion[_docHash] = newVersion;
        emit VersionCreated(_docHash, newVersion, _contentHash);
    }
}
\end{lstlisting}

\subsection{Signing.sol}

Gestiona las firmas digitales sobre versiones de documentos, registrando la direccion del firmante y el instante de la firma.

\begin{lstlisting}[language=Java, caption={Signing.sol -- Firmas digitales}]
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;
...
contract Signing {
    ...
    function signDocument(
        ...
        signatures[_docHash].push(Signature({
            signer: msg.sender,
            docHash: _docHash,
        ...
        emit DocumentSigned(_docHash, msg.sender, _versionNumber);
    }
}
\end{lstlisting}

\subsection{AccessControl.sol}

Control de acceso y permisos sobre documentos, diferenciando entre ausencia de permiso, lectura, escritura y firma.

\begin{lstlisting}[language=Java, caption={AccessControl.sol -- Control de acceso}]
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;

contract AccessControl {
    enum Permission { NONE, READ, WRITE, SIGN }
    ...
    function grantPermission(
        ...
        permissions[_docHash][_user] = _permission;
        emit PermissionGranted(_docHash, _user, _permission);
    }

    function revokePermission(bytes32 _docHash, address _user) external {
        delete permissions[_docHash][_user];
        emit PermissionRevoked(_docHash, _user);
    }
}
\end{lstlisting}
...
Durante la fase de diseno se evaluo la opcion de mantener la logica distribuida en cuatro contratos independientes (registro, versionado, firmas y permisos) frente a la consolidacion en un unico contrato \texttt{DocumentRegistry}. Se opto por un solo contrato sobre todo por simplicidad. Ademas ayuda a reducir el gas del despliegue y evita problemas de atomicidad.
...
\subsection{Verificacion funcional del contrato inteligente}

Se implemento una bateria de 102 tests funcionales sobre el contrato \texttt{DocumentRegistry} que verifican la correcta emision de los diez eventos definidos, el control de acceso basado en roles mediante \texttt{AccessControl} de OpenZeppelin, la proteccion contra reentrancia (\texttt{ReentrancyGuard}), y los casos limite aritmeticos (direccion cero, valores maximos de \texttt{uint256}). Los tests cubren el despliegue, la creacion de documentos, la gestion completa de versiones (crear, restaurar y establecer version operativa), la firma digital, la comparticion y revocacion de permisos, la transferencia de propiedad, el archivado y borrado de documentos, y las funciones de solo lectura que recuperan el estado on-chain. Esta bateria constituye una verificacion funcional exhaustiva del componente on-chain del sistema y se ejecuta mediante el entorno Hardhat con una red local de pruebas.
