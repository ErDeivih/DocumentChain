...
La consistencia entre la base de datos relacional y la blockchain se mantiene mediante tres mecanismos complementarios: una cache de estado on-chain, un listener de eventos en tiempo real y un worker de reconciliacion periodica.

\textbf{Cache de estado on-chain.} El servicio \texttt{BlockchainCacheService} (en \texttt{backend/src/services/blockchainCacheService.ts}) mantiene una cache en memoria con TTL de 30 segundos del estado de cada documento segun el contrato: propietario, estado de archivado, borrado y version operativa. Esto evita consultar la blockchain para cada verificacion de permisos, reduciendo la latencia de las operaciones de lectura.

\textbf{Event Listener.} Al iniciarse, el backend suscribe una instancia del contrato a los eventos reales del contrato \texttt{DocumentRegistry}: \texttt{DocumentCreated}, \texttt{VersionCreated}, \texttt{VersionRestored}, \texttt{DocumentSigned}, \texttt{DocumentShared}, \texttt{PermissionRevoked}, \texttt{OwnershipTransferred}, \texttt{DocumentArchived}, \texttt{DocumentDeleted} y \texttt{OperationalVersionChanged}. Cuando se emite un evento, el callback invalida la entrada correspondiente en la cache y actualiza el estado en PostgreSQL.

\textbf{Reconciliacion periodica.} El worker \texttt{BlockchainReconciler} (en \texttt{backend/src/workers/blockchainReconciler.ts}) se ejecuta cada seis horas y recorre los documentos sincronizados comparando su estado en base de datos con el estado on-chain obtenido via la cache. Si detecta divergencias, corrige el registro local para mantener la coherencia entre ambas fuentes de datos.
...
\begin{itemize}
    ...
    \item \textbf{ipfs-node (opcional):} Nodo Kubo oficial de IPFS. Aparece comentado en el \texttt{docker-compose.yml} por defecto, ya que el sistema utiliza Pinata como proveedor IPFS principal. Si se habilita, gestiona el almacenamiento distribuido de archivos, el pinning local y la API HTTP en el puerto 5001.
    ...
    \item \textbf{backend:} Servicio Node.js que encapsula la API REST. Depende de \texttt{postgres} y, si el nodo Kubo esta habilitado, de \texttt{ipfs-node}. El backend se comunica con Pinata como proveedor IPFS por defecto. Se reconstruye automaticamente ante cambios en el codigo fuente durante el desarrollo.
...
\textbf{Volúmenes.} Se definen volúmenes Docker persistentes para \texttt{postgres-data} (ficheros de base de datos) y \texttt{backend-logs} (ficheros de trazas de Winston). Si se habilita el nodo Kubo opcional, se añade el volumen \texttt{ipfs-data} para el repositorio de bloques y configuracion del nodo. Esta separacion garantiza que los datos sobrevivan a la recreacion de contenedores.
...
\begin{description}
    ...
    \item[Hardhat] Entorno de desarrollo local de Ethereum utilizado para compilar, desplegar y probar contratos inteligentes en una red simulada antes de un despliegue en mainnet.
