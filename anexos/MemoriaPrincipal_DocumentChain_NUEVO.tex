...
\begin{description}[style=nextline]
    ...
    \item[EIP-712] Propuesta de mejora de Ethereum para la firma tipada de datos estructurados.
...
\hline
Registro inmutable & Smart contracts con hashes SHA-256 y firmas EIP-712 & Trazabilidad verificable sin intermediarios \\
\hline
...
\begin{itemize}
    ...
    \item Que el sistema permita firmar documentos mediante wallets compatibles con EIP-6963, generando mensajes estructurados segun EIP-712 y registrando la firma en blockchain.
...
\noindent \textbf{Node.js y Express 5} proporcionan el backend. La naturaleza asincrona de Node.js se adapta bien a las operaciones con IPFS y blockchain, mientras que Express facilita el enrutamiento y la gestion de middlewares.
...
A modo de ejemplo, la Tabla \ref{tab:obj-func} muestra los objetivos funcionales del sistema, la Tabla \ref{tab:actores} identifica los actores que interactuan con DocumentChain, y la Figura \ref{fig:usecase-general-mem} presenta el diagrama general de casos de uso con la distribucion de los 39 casos de uso en siete paquetes funcionales (Tabla \ref{tab:uc-paquetes}).

A titulo ilustrativo, la Tabla \ref{tab:uc-ejemplo} muestra la especificacion completa de un caso de uso representativo del sistema: UC-0013 Subir Documento, que constituye la operacion principal del modulo de gestion documental e involucra las tres capas de persistencia (PostgreSQL, IPFS y blockchain). El Anexo I contiene las 39 especificaciones completas con este mismo nivel de detalle.
...
\hline
\end{tabular}
\caption{Ejemplo de especificacion de caso de uso: UC-0013 Subir Documento}
...
A modo de ejemplo, la Figura \ref{fig:seq-bce-ejemplo} muestra el diagrama de secuencia de analisis BCE correspondiente al caso de uso UC-0013 (Subir Documento), que ilustra la aplicacion del patron Boundary-Control-Entity. La Figura \ref{fig:seq-design-ejemplo} presenta el diagrama de secuencia de diseno para el mismo caso de uso, reflejando la implementacion real con React (frontend), Express (backend), IPFS y el smart contract.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\linewidth]{diagramas/seq-bce-uc0013-upload.png}
    \caption{Diagrama de secuencia de analisis BCE: UC-0013 Subir Documento}
...
\begin{figure}[H]
    ...
    \caption{Diagrama de secuencia de diseno: UC-0013 Subir Documento}
